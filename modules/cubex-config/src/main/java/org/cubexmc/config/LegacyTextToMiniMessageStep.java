...
public final class LegacyTextToMiniMessageStep implements MigrationStep {
    ...
    public String description() {
        return "Convert legacy &/hex/placeholder text to MiniMessage.";
    }

    @Override
    public void migrate(MigrationContext context) {
        convertSection(context, context.yaml(), "");
    }
    ...
    private void convertSection(MigrationContext context, ConfigurationSection section, String basePath) {
        for (String key : section.getKeys(false)) {
            String path = basePath.isEmpty() ? key : basePath + "." + key;
            if (section.isConfigurationSection(key)) {
                convertSection(context, section.getConfigurationSection(key), path);
    ...
    private int appendLegacyTag(String input, int ampersandIndex, StringBuilder output) {
        if (ampersandIndex + 1 >= input.length()) {
            return ampersandIndex;
        }
        if (input.charAt(ampersandIndex + 1) == '#'
        ...
        char code = Character.toLowerCase(input.charAt(ampersandIndex + 1));
