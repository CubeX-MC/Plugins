...
- `OPEN` 到期无人接单：奖金退回雇主。
- `IN_PROGRESS` 到期未提交：奖金退回雇主，接单者声望扣分。
- `SUBMITTED` 到期未确认：建议自动确认，或进入管理员待处理队列。
...
- 合同大厅：展示公开合同，支持分页。
- 筛选菜单：类型、奖金范围、状态、是否即将到期。
- 创建向导：类型、标题、描述、奖金、截止时间、地点。
